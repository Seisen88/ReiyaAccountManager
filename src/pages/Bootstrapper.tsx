...
const LAUNCHER_OPTIONS = [
  ...
  { id: "reiya",     name: "Reiya",           subtitle: "Built-in",    desc: "Runs Reiya's standalone Roblox build directly.",      accentColor: "var(--accent)", alwaysAvailable: false },
...
function BootstrapperTab({ flagsCount, onSwitchTab }: { flagsCount: number; onSwitchTab: () => void }) {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{
        padding: "24px 28px",
        background: `linear-gradient(135deg, ${statusColor}08 0%, var(--panel-bg) 60%)`,
        borderBottom: "1px solid var(--g06)",
        flexShrink: 0,
      }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ width: 1, height: 24, background: "var(--g08)" }} />
        ...
        {(installing || (progress && !progress.done)) && progress && (
          <div style={{ marginTop: 18, padding: "14px 18px", background: "var(--g02)", borderRadius: 12, border: "1px solid var(--g06)" }}>
            ...
            <div style={{ height: 5, background: "var(--g04)", borderRadius: 99, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              ...
              <button onClick={scanInstalls} disabled={detecting} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 6, border: "1px solid var(--g06)", background: "transparent", color: "var(--t3)", fontSize: 10, fontWeight: 700, cursor: detecting ? "not-allowed" : "pointer" }}
          ...
          <div>
            ...
            <div style={{ marginTop: 10 }}>
              {[
                ...
                <div key={n} style={{ display: "flex", gap: 10, padding: "7px 0", borderBottom: "1px solid var(--g03)" }}>
                  <span style={{ fontSize: 9, fontWeight: 900, color: "var(--g12)", fontFamily: "monospace", minWidth: 16 }}>{n}</span>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: isSelected ? accentColor + "07" : hov ? "var(--g02)" : "var(--g01)",
        border: `1px solid ${isSelected ? accentColor + "50" : hov ? "var(--g07)" : "var(--g04)"}`,
    ...
    >
      ...
      <div style={{ width: 34, height: 34, borderRadius: 9, flexShrink: 0, background: isSelected ? accentColor + "18" : "var(--g04)", border: `1px solid ${isSelected ? accentColor + "35" : "var(--g06)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
          ...
          <span style={{ fontSize: 8.5, color: "var(--t3)", background: "var(--g04)", padding: "1px 5px", borderRadius: 4, fontWeight: 600 }}>{subtitle}</span>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
        <div style={{ width: 16, height: 16, borderRadius: "50%", border: `2px solid ${isSelected ? accentColor : "var(--g15)"}`, background: isSelected ? accentColor : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all .12s" }}>
          {isSelected && <div style={{ width: 5, height: 5, borderRadius: "50%", background: "var(--bg)" }} />}
        </div>
        <span style={{ fontSize: 7.5, fontWeight: 800, padding: "1.5px 5px", borderRadius: 4, color: available ? "var(--green)" : "var(--t3)", background: available ? "rgba(52,211,153,0.08)" : "var(--g03)" }}>
          {available ? "READY" : "N/A"}
        </span>
      </div>
    </div>
...
function InstallCard({ install }: { install: RobloxInstall }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", background: "var(--g01)", borderRadius: 10, border: `1px solid ${install.found ? "var(--g05)" : "var(--g02)"}`, opacity: install.found ? 1 : 0.45 }}>
      <div style={{ width: 26, height: 26, borderRadius: 7, flexShrink: 0, background: install.found ? color + "12" : "var(--g02)", border: `1px solid ${install.found ? color + "28" : "var(--g04)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <span style={{ fontSize: 8, fontWeight: 800, padding: "2px 6px", borderRadius: 4, color: install.found ? "var(--green)" : "var(--t3)", background: install.found ? "rgba(52,211,153,0.08)" : "var(--g03)", flexShrink: 0 }}>
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <div style={{ width: 1, height: 24, background: "var(--g08)", alignSelf: "center" }} />
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        {/* Presets panel */}
        <div style={{ width: 256, flexShrink: 0, borderRight: "1px solid var(--g05)", display: "flex", flexDirection: "column", background: "rgba(8,9,12,0.3)" }}>
          <div style={{ padding: "12px 12px 8px", borderBottom: "1px solid var(--g05)" }}>
            <div style={{ fontSize: 9.5, fontWeight: 800, color: "var(--t3)", letterSpacing: "0.12em", marginBottom: 8 }}>PRESET CATEGORIES</div>
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} style={{ padding: "3px 9px", borderRadius: 7, border: "none", fontSize: 9.5, fontWeight: 700, cursor: "pointer", transition: "all .15s", background: activeCategory === cat ? "rgba(167,139,250,0.14)" : "var(--g03)", color: activeCategory === cat ? "#A78BFA" : "var(--t2)" }}>{cat}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          {/* Toolbar */}
          <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--g05)", background: "rgba(8,9,12,0.2)", display: "flex", alignItems: "center", gap: 9 }}>
          ...
          <div className="scroll" style={{ flex: 1, padding: "14px 16px" }}>
            {/* Add new */}
            <div style={{ display: "flex", gap: 8, marginBottom: 12, padding: "10px 12px", background: "var(--g01)", borderRadius: 10, border: "1px solid var(--g05)" }}>
              ...
              <button onClick={addFlag} className="glow-btn" style={{ padding: "7px 16px", borderRadius: 8, border: "none", background: "var(--accent)", color: "var(--accent-text)", fontSize: 11.5, fontWeight: 800, cursor: "pointer" }}
          ...
          {/* Save footer */}
          <div style={{ padding: "11px 16px", borderTop: "1px solid var(--g05)", background: "rgba(8,9,12,0.4)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
            ...
            <button onClick={handleSave} disabled={saving} className="glow-btn" style={{ padding: "8px 20px", borderRadius: 9, border: "none", background: saving ? "var(--g08)" : "var(--accent)", color: saving ? "var(--t3)" : "#0a0a0a", fontSize: 12, fontWeight: 800, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1, boxShadow: "0 4px 14px rgba(232,232,232,.15)", display: "flex", alignItems: "center", gap: 6 }}
...
function FlagRow({ flagKey, value, onChange, onRemove }: { flagKey: string; value: any; onChange: (v: string) => void; onRemove: () => void }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--g01)", borderRadius: 8, border: "1px solid var(--g04)", transition: "border-color .15s" }}
      onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.borderColor = "var(--g08)"}
      onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.borderColor = "var(--g04)"}>
      ...
      {isBool
        ? <div onClick={() => onChange(value ? "false" : "true")} style={{ width: 34, height: 18, borderRadius: 99, background: value ? "var(--g15)" : "var(--g04)", border: `1px solid ${value ? "var(--g30)" : "var(--g08)"}`, position: "relative", cursor: "pointer", transition: "all .15s", flexShrink: 0 }}>
            <div style={{ position: "absolute", top: 2, left: value ? 17 : 2, width: 12, height: 12, borderRadius: "50%", background: value ? "var(--accent)" : "#8B8FA8", transition: "all .15s" }} />
          </div>
      ...
      <span style={{ fontSize: 8.5, padding: "2px 6px", borderRadius: 4, background: "var(--g03)", color: "var(--t3)", fontWeight: 800, flexShrink: 0 }}>{typeof value}</span>
